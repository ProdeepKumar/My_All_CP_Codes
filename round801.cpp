...
void solve()
{
    ...
    mem(dp,0);
    dp[0][0]=a[0][0];
    f(i,1,n) dp[i][0]+=dp[i-1][0]+a[i][0];
    f(i,1,m) dp[0][i]+=dp[0][i-1]+a[0][i];
    f(i,1,n) f(j,1,m) dp[i][j]=a[i][j]+min(dp[i-1][j],dp[i][j-1]);
    mn=dp[n-1][m-1];
    mem(dp,0);
    dp[0][0]=a[0][0];
    f(i,1,n) dp[i][0]+=dp[i-1][0]+a[i][0];
    f(i,1,m) dp[0][i]+=dp[0][i-1]+a[0][i];
    f(i,1,n) f(j,1,m) dp[i][j]=a[i][j]+max(dp[i-1][j],dp[i][j-1]);
    mx=dp[n-1][m-1];
    // debug(mn);
    // debug(mx);
    if(mn<=0 and mx>=0) coy
    else con
}

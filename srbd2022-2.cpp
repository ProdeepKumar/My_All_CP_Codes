...
void solve()
{
    ...
    while(q--)
    {
        ...
        else
        {
            l=r=1e15;
            if(y<n) l=abs(x-a[y]);
            if(y>0) r=abs(x-a[y-1]);
            // debug(l);
            // debug(r);
            if(l==1e15 and r!=1e15)
            {
                res=*mp[a[y-1]].begin();
            }
            else if(l!=1e15 and r==1e15)
            {
                res=*mp[a[y]].begin();
            }
            else if(l!=1e15 and r!=1e15)
            {
                if(l<r) res=*mp[a[y]].begin();
                else if(l>r) res=*mp[a[y-1]].begin();
                else
                {
                    res=min(*mp[a[y]].begin(),*mp[a[y-1]].begin());
                }
            }
        }
        co<<res<<endl;

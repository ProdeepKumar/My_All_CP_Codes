...
{
    ci>>n;
    vector<pi>a(n+1),ab,ba;
    vector<ll>ans(n+1),b(n+1);
    f(i,1,n+1) 
    {
        ci>>x;
        b[i]=x;
    ...
    f(i,1,n+1)
    {
        if(a[i].fi!=0) ab.pb(a[i]);
        else ba.pb(a[i]);
    }
    sort(allr(ab));
    sort(all(ba));
    f(i,0,ba.size()) ab.pb(ba[i]);
    // sort(allr(a));
    // for(auto ii:ab) co<<ii.fi<<' '<<ii.se<<endl;
    f(i,0,n) ans[ab[i].se]=i+1;
    // f(i,1,n+1) co<<ans[i]<< ' ';
    // en
    r(i,n,2)
    {
        if((i/ans[i])!=b[i])
        {
            f(j,1,n+1)
            {
                if((j/ans[i])==b[j] and (i/ans[j])==b[i] and i!=j)
                {
                    swap(ans[i],ans[j]);
                    br
                }

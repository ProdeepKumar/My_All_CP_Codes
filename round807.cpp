...
void solve()
{
    ...
    vector<ll>ab;
    mp.clear();
    f(i,1,n+1) mp[i]=i,ab.pb(i);
    ll now=n+1;
    while (c--)
    {
        ci>>x>>y;
        if(binary_search(all(ab),x))
        {
            ll id=lower_bound(all(ab),x)-ab.begin();
            mp[now]=mp[ab[id]];
        }
        else
        {
            ll id=upper_bound(all(ab),x)-ab.begin();
            id--;
            mp[now]=mp[ab[id]]+x-ab[id];
        }
        if(binary_search(all(ab),y))
        {
            ll id=lower_bound(all(ab),y)-ab.begin();
            mp[now+y-x]=mp[ab[id]];
        }
        else
        {
            ll id=upper_bound(all(ab),y)-ab.begin();
            id--;
            mp[now+y-x]=mp[ab[id]]+y-ab[id];
        }
        ab.pb(now);
        ab.pb(now+y-x);
        now+=y-x+1;
    ...
    // for(ll ii:ab) co<<ii<<' ';
    // en
    // for(auto ii:mp) co<<ii.fi<<' '<<ii.se<<endl;
    // co<<a<<endl;
    while(q--)
    {
        ci>>x;
